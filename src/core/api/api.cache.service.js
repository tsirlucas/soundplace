...
const appCachePromise = window.caches
  ? window.caches.open(CACHE_ID)
...
export const set = (key, response) => appCachePromise.then((cache) => cache.put(key, response));

export const get = (key) => appCachePromise.then((cache) => cache.match(key));

...
const storageEpic = (action$) =>
  action$
    .ofType(LOAD_STORAGE_STATUS)
    .mergeMap(estimateStorage)
    .map(loadStorageStatusSuccess);

const cachedSongsEpic = (action$) =>
  action$
    .ofType(GET_CACHED_SONGS)
    .mergeMap(getCachesKeys)
    .mergeMap(getCaches)
    .mergeMap(getCacheRequests)
    .map(concatRequests)
    .map(filterSongsReq)
    .mergeMap((songs) => parseSongs(songs).map(getCachedSongsSuccess));

const saveMusicEpic = (action$) =>
  action$
    .ofType(SAVE_MUSIC)
    .mergeMap(({payload}) => fetchMusic(payload.track).map(() => saveMusicSuccess(payload)));

const deleteMusicEpic = (action$) =>
  action$.ofType(DELETE_MUSIC).mergeMap(({payload}) =>
    getCachesKeys()
      .mergeMap(getCaches)
      .mergeMap((cacheRes) => deleteCacheRequests(cacheRes, payload))
      .mergeMap(() =>
        Observable.concat(Observable.of(loadStorageStatus()), Observable.of(getCachedSongs())),
      ),
  );

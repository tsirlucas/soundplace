...
const importUserDataEpic = (action$) => {
  return action$.ofType(IMPORT_USER_DATA).mergeMap(() => importUserData().map(reloadRoute));
};

const getUserEpic = (action$) => {
  return action$.ofType(GET_USER_REQUEST).mergeMap(() => getUser().map(getUserSuccess));
};

...
importScripts(
  "precache-manifest.16ae5f48088c660c407b1d2e1b648db9.js"
);
...
workbox.routing.registerRoute(/^https:\/\/api-soundplace\.com\/stream\//, workbox.strategies.cacheFirst({ cacheName: "stream-cache", plugins: [{ cacheWillUpdate: ({ request, response }) => { caches.match(request.url, { ignoreSearch: true }).then(res => { if (res) { return null; } if (request.url.includes('?save=true')) { return response; } return null; }); }, cachedResponseWillBeUsed: ({ request, cachedResponse }) => { if (cachedResponse) { return cachedResponse; } return caches.match(request.url, { ignoreSearch: true }); } }, new workbox.expiration.Plugin({"maxEntries":100000,"maxAgeSeconds":31536000,"purgeOnQuotaError":false}), new workbox.cacheableResponse.Plugin({"statuses":[0,200,201,206,301,304,302]})] }), 'GET');
